...
function resetForm() {
    ...
    // Clear selections from sessionStorage
    sessionStorage.removeItem("pcBuildSelections");
}
...
// Function to save current selections to sessionStorage
function saveSelectionsToSessionStorage() {
    const categories = [
        "CPU", "Mainboard", "RAM", "SSD", "HDD",
        "VGA", "PSU", "Case", "AirCooler", "LiquidCooler"
    ];
    ...
    sessionStorage.setItem("pcBuildSelections", JSON.stringify(selections));
}

// Event listener for when the page loads
window.addEventListener("load", () => {
    const data = sessionStorage.getItem("pcBuildSelections");
    if (data) {
...
function submitBuildToCart() {
    const selections = JSON.parse(sessionStorage.getItem("pcBuildSelections") || "{}");
    const cartItems = [];
    ...
    fetch("/Cart/AddBuildToCart", {
        ...
        if (res.ok) {
            sessionStorage.removeItem("pcBuildSelections"); // Clear sessionStorage after successful submission
